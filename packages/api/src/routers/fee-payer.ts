...
import { getBalance, getFeePayer } from '@solana-stack-attack/solana-client'
...
export const feePayerRouter = {
  getBalance: publicProcedure.handler(async ({ context }) => {
    ...
    if (!env.FEE_PAYER_KEYPAIR) {
      return { address: null, lamports: 0, sol: 0, funded: false }
    }

    const feePayer = await getFeePayer(env.FEE_PAYER_KEYPAIR)
    const result = await getBalance(context.solana, feePayer.address)
