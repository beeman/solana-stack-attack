...
A tap-to-earn mobile game built on Solana that rewards players with real tokens for hitting milestones. Built for the Solana Mobile Hackathon.
...
Players earn **STACK** (SPL token on Solana devnet) for hitting milestones:
...
Powered by [better-auth](https://www.better-auth.com/) with a custom SIWS plugin.
...
│   │       ├── profile/ # Player profile with wallet
│   │       └── solana/  # Wallet connection, balance
...
| `BETTER_AUTH_URL` | Server URL for auth | `http://localhost:3000` |
...
Built by [colmena](https://github.com/colmena-dev) — **@beeman** and **@obrera** (AI pair programmer 🐝)
